...
fn main() {
    ...
    .expect("generate simple UUID token");
    println!("   Token: {}", token.as_str());
    println!(
        "   长度 | Length: {} 字符 | characters\n",
        token.as_str().len()
    ...
    .expect("generate random-32 token");
    println!("   Token: {}", token.as_str());
    println!("   长度: {} 字符\n", token.as_str().len());
    ...
    .expect("generate random-64 token");
    println!("   Token: {}", token.as_str());
    println!("   长度: {} 字符\n", token.as_str().len());
    ...
    let token = TokenGenerator::generate_with_login_id(&config_timestamp, test_login_id)
        .expect("generate timestamp token");
    println!("   Token: {}", token.as_str());
    println!("   长度: {} 字符", token.as_str().len());
    println!("   说明: 毫秒级时间戳_16位随机字符\n");

    // 7. Tik 风格（新）
    println!("7. Tik 风格 (Java-compatible 2_14_16__):");
    ...
    let token = TokenGenerator::generate_with_login_id(&config_tik, test_login_id)
        .expect("generate Tik token");
    println!("   Token: {}", token.as_str());
    println!("   长度: {} 字符", token.as_str().len());
    println!("   说明: Java 兼容的 2_14_16__ 结构（URL安全）\n");
    ...
    println!("Random32   | 32 字符 | CSPRNG Base62 随机字符串");
    println!("Random64   | 64 字符 | CSPRNG Base62 随机字符串（更长）");
    println!("Hash       | 64 字符 | SHA256 哈希，包含用户信息");
    println!("Timestamp  | ~30字符 | 包含时间信息，便于追溯");
    println!("Tik        | 36 字符 | Java 兼容 2_14_16__ 结构");
