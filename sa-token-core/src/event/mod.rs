...
//!     async fn on_login(&self, _login_id: &str, _token: &str, login_type: &str) {
//!         println!("login event, type: {}", login_type);
//!         // 不要将 login_id 或 token 写入日志
//!     }
//!     
//!     async fn on_logout(&self, _login_id: &str, _token: &str, login_type: &str) {
//!         println!("logout event, type: {}", login_type);
//!     }
//! }
//!
        ...
        for listener in listeners.iter() {
            ...
            match event.event_type {
                ...
                SaTokenEventType::RenewTimeout => {
                    listener
                        .on_renew_timeout(&event.login_id, &event.token, &event.login_type)
                        .await;
                }
                ...
                SaTokenEventType::CloseSafe => {
                    listener
                        .on_close_safe(&event.token, &event.login_type)
                        .await;
                }
            }
        }
...
impl SaTokenListener for LoggingListener {
    ...
    async fn on_renew_timeout(&self, _login_id: &str, _token: &str, login_type: &str) {
        tracing::debug!(
            event = "renew_timeout",
            login_type = %login_type,
            "Token 续期"
    ...
    async fn on_replaced(&self, _login_id: &str, _token: &str, login_type: &str) {
        tracing::warn!(
            event = "replaced",
            login_type = %login_type,
            "用户被顶下线"
    ...
    async fn on_banned(&self, _login_id: &str, login_type: &str) {
        tracing::warn!(
            event = "banned",
            login_type = %login_type,
            "用户被封禁"
