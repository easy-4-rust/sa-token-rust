...
impl OAuth2Manager {
    ...
    pub async fn register_client(&self, client: &OAuth2Client) -> SaTokenResult<()> {
        ...
        let value = serde_json::to_string(client).map_err(SaTokenError::SerializationError)?;
    ...
    pub async fn verify_client(&self, client_id: &str, client_secret: &str) -> SaTokenResult<bool> {
        ...
        Ok(crate::constant_time::ct_eq_str(
    ...
    pub async fn store_authorization_code(
        ...
        let value = serde_json::to_string(auth_code).map_err(SaTokenError::SerializationError)?;
    ...
    pub async fn generate_access_token(
        ...
        let refresh_ttl = Some(std::time::Duration::from_secs(
            self.refresh_token_ttl as u64,
        ));
        self.storage
            .set(&refresh_key, &refresh_value, refresh_ttl)
            .await
            .map_err(|e| SaTokenError::StorageError(e.to_string()))?;
    ...
    pub async fn refresh_access_token(
        ...
        // 3. Validate client ID matches
        let stored_client_id = data["client_id"]
            .as_str()
        ...
        let scope: Vec<String> = data["scope"]
            .as_array()
    ...
    #[allow(clippy::too_many_arguments)] // Java-compatible grant dispatch facade.
...
mod tests {
    ...
    async fn test_refresh_token() {
        ...
        let token = oauth2
            .generate_access_token("test_client", "user_123", vec!["read".to_string()])
            .await
            .unwrap();

        let refresh_token = token.refresh_token.as_ref().unwrap();
        let new_token = oauth2
            .refresh_access_token(refresh_token, "test_client", "test_secret")
            .await
            .unwrap();
