...
impl SaStorage for RedisStorage {
    ...
    async fn set_if_absent(
        ...
        command.arg(self.full_key(key)).arg(value).arg("NX");
        if let Expiration::After(ttl) = expiration {
            command.arg("PX").arg(Self::ttl_millis(ttl));
        }

        let mut conn = self.client.clone();
        let response: Option<String> = command
            .query_async(&mut conn)
            .await
            .map_err(|error| StorageError::OperationFailed(error.to_string()))?;
        Ok(response.is_some())
    ...
    async fn ttl_state(&self, key: &str) -> StorageResult<TtlState> {
        ...
        match ttl_millis {
            ...
            millis if millis >= 0 => Ok(TtlState::ExpiresIn(Duration::from_millis(millis as u64))),
            unexpected => Err(StorageError::OperationFailed(format!(
                "unexpected Redis PTTL response: {unexpected}"
            ))),
        }
    }
    ...
    async fn clear(&self) -> StorageResult<()> {
        ...
        loop {
            let page = self.scan("*", cursor.as_deref(), 512).await?;
            keys.extend(page.keys.into_iter().map(|key| self.full_key(&key)));
    ...
    async fn keys(&self, pattern: &str) -> StorageResult<Vec<String>> {
        ...
        let mut keys = Vec::new();
        loop {
            let page = self.scan(pattern, cursor.as_deref(), 512).await?;
    ...
    async fn scan(
        ...
        let cursor = cursor.unwrap_or("0").parse::<u64>().map_err(|error| {
