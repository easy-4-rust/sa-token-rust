...
impl auth::auth_service_server::AuthService for AuthServiceImpl {
    ...
    async fn login(
        ...
        let get_demo_password = |username: &str| -> Option<String> {
            match username {
                "admin" => Some(
                    std::env::var("DEMO_ADMIN_PASSWORD").unwrap_or_else(|_| "admin123".to_string()),
                ),
                "user" => Some(
                    std::env::var("DEMO_USER_PASSWORD").unwrap_or_else(|_| "user123".to_string()),
                ),
                "guest" => Some(
                    std::env::var("DEMO_GUEST_PASSWORD").unwrap_or_else(|_| "guest123".to_string()),
                ),
                _ => None,
            }
        };
