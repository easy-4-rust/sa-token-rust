...
use warp_03::{Filter, Rejection, Reply, reply};
    ...
    T: Send + 'static,
{
    ...
    filter::sa_token_filter(state)
        .and(inner)
        .and_then(move |token_data: TokenData, value: T| {
            let mgr = mgr.clone();
            async move {
...
pub fn sa_check_login(
    ...
    warp_03::any().map(|| reply::json(&serde_json::json!({"login_id": "user"})))
}
